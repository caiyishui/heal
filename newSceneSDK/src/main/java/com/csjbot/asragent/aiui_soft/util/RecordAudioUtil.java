...
public class RecordAudioUtil {
    ...
    //2mic通道适配 2通道-16k-16bit 转 4通道-16k-32bit
    //回采信号置空
    ...
    //2mic通道适配 4c-48k-16bit 转 4c-16k-32bit
    ...
    //2mic通道适配 24k 16bit 转 16k 32bit
    public static byte[] addCnFor2MicN3(byte[] data) {
        byte[] cpy = new byte[data.length * 2 / 3 * 2];
        int j = 0;
        // 每三帧取前两帧，回采复用第三通道
        //通道： mic1 mic2 ref ref
        while (j < data.length / 24) {
            ...
            cpy[16 * j + 12] = 00;
            cpy[16 * j + 13] = 00;
            cpy[16 * j + 14] = data[24 * j + 4];
            cpy[16 * j + 15] = data[24 * j + 5];

            cpy[16 * (j + 1)] = 00;
            cpy[16 * (j + 1) + 1] = 00;
            cpy[16 * (j + 1) + 2] = data[24 * j + 8];
            cpy[16 * (j + 1) + 3] = data[24 * j + 9];

            cpy[16 * (j + 1) + 4] = 00;
            cpy[16 * (j + 1) + 5] = 00;
            cpy[16 * (j + 1) + 6] = data[24 * j + 10];
            cpy[16 * (j + 1) + 7] = data[24 * j + 11];

            cpy[16 * (j + 1) + 8] = 00;
            cpy[16 * (j + 1) + 9] = 00;
            cpy[16 * (j + 1) + 10] = data[24 * j + 12];
            cpy[16 * (j + 1) + 11] = data[24 * j + 13];

            cpy[16 * (j + 1) + 12] = 00;
            cpy[16 * (j + 1) + 13] = 00;
            cpy[16 * (j + 1) + 14] = data[24 * j + 12];
            cpy[16 * (j + 1) + 15] = data[24 * j + 13];
            j += 2;
        }
        return cpy;
    }
    ...
    //2mic通道适配 16k 16bit 转 16k 32bit
    public static byte[] addCnFor2MicF4(byte[] data) {
        byte[] cpy = new byte[data.length * 2];
        int j = 0;
    ...
    //2mic通道适配 2c-48k-16bit 转 4c-16k-16bit
    public static byte[] addCnFor2MicN5(byte[] data) {
        byte[] cpy = new byte[data.length * 2 / 3];
        int j = 0;
        // 每三帧取前一帧
        //通道： mic1 mic2 ref ref
        while (j < data.length / 12) {
            ...
            cpy[8 * j + 0] = data[12 * j + 0];
            cpy[8 * j + 1] = data[12 * j + 1];

            cpy[8 * j + 2] = data[12 * j + 2];
            cpy[8 * j + 3] = data[12 * j + 3];
    ...
    private static byte[] adjusAudio(byte[] data) {
    ...
    //4mic通道适配 48k-16bit-6通道 转 16k-16bit-6通道
    public static byte[] addCnFor4MicN1(byte[] data) {
        // 查找第一的通道号
        int index = 0;
        for (index = 0; index < 24; index = index + 4) {
            if ((data[index + 1] & 0x0f) == 1) {
                break;
            }
        }
        byte[] cpy = new byte[data.length / 3];
        int j = 0;

        //通道： mic1 mic2 mic3 mic4 ref ref
        while (j < data.length / 36) {
            cpy[12 * j + 0] = data[36 * j + 0];
            cpy[12 * j + 1] = data[36 * j + 1];

            cpy[12 * j + 2] = data[36 * j + 2];
            cpy[12 * j + 3] = data[36 * j + 3];

            cpy[12 * j + 4] = data[36 * j + 4];
            cpy[12 * j + 5] = data[36 * j + 5];

            cpy[12 * j + 6] = data[36 * j + 6];
            cpy[12 * j + 7] = data[36 * j + 7];

            cpy[12 * j + 8] = data[36 * j + 8];
            cpy[12 * j + 9] = data[36 * j + 9];

            cpy[12 * j + 10] = data[36 * j + 10];
            cpy[12 * j + 11] = data[36 * j + 11];
    ...
    //4mic通道适配 8通道-48k-16bit 转 6通道-16k-16bit
    //最后两通道丢弃
    public static byte[] addCnFor4MicN2(byte[] data) {
        int size = (data.length) / 8 * 6 / 3;
        byte[] cpy = new byte[size];
        int j = 0;
        // 每三帧取第一帧
        // 6通道： mic1 mic2 mic3 mic4 ref ref
        while (j < data.length / 48) {
            cpy[12 * j + 0] = data[48 * j + 0];
            cpy[12 * j + 1] = data[48 * j + 1];

            cpy[12 * j + 2] = data[48 * j + 2];
            cpy[12 * j + 3] = data[48 * j + 3];

            cpy[12 * j + 4] = data[48 * j + 4];
            cpy[12 * j + 5] = data[48 * j + 5];

            cpy[12 * j + 6] = data[48 * j + 6];
            cpy[12 * j + 7] = data[48 * j + 7];

            cpy[12 * j + 8] = data[48 * j + 8];
            cpy[12 * j + 9] = data[48 * j + 9];

            cpy[12 * j + 10] = data[48 * j + 10];
            cpy[12 * j + 11] = data[48 * j + 11];
    ...
    //4mic通道适配 8通道-48k-16bit 转 6通道-16k-16bit
    //最后两通道丢弃
    public static byte[] addCnFor4MicN3(byte[] data) {
        int size = (data.length) / 8 * 6 / 3;
        byte[] cpy = new byte[size];
        int j = 0;
        // 每三帧取第一帧
        // 6通道： mic1 mic2 mic3 mic4 ref ref
        while (j < data.length / 48) {
            cpy[12 * j + 0] = data[48 * j + 2];
            cpy[12 * j + 1] = data[48 * j + 3];

            cpy[12 * j + 2] = data[48 * j + 0];
            cpy[12 * j + 3] = data[48 * j + 1];

            cpy[12 * j + 4] = data[48 * j + 6];
            cpy[12 * j + 5] = data[48 * j + 7];

            cpy[12 * j + 6] = data[48 * j + 4];
            cpy[12 * j + 7] = data[48 * j + 5];
    ...
    public static byte[] adapeter4Mic1(byte[] data) {
        ...
        while (j < data.length / 16) {
            cpy[24 * j + 0] = 0x00;
            cpy[24 * j + 1] = 0x01;
            cpy[24 * j + 2] = data[16 * j + 2];
            cpy[24 * j + 3] = data[16 * j + 3];

            cpy[24 * j + 4] = 0x00;
            cpy[24 * j + 5] = 0x02;
            cpy[24 * j + 6] = data[16 * j + 4];
            cpy[24 * j + 7] = data[16 * j + 5];

            cpy[24 * j + 8] = 0x00;
            cpy[24 * j + 9] = 0x03;
            cpy[24 * j + 10] = data[16 * j + 6];
            cpy[24 * j + 11] = data[16 * j + 7];

            cpy[24 * j + 12] = 0x00;
            cpy[24 * j + 13] = 0x04;
            cpy[24 * j + 14] = data[16 * j + 8];
            cpy[24 * j + 15] = data[16 * j + 9];

            //通道5--》ref1
            cpy[24 * j + 16] = 0x00;
            cpy[24 * j + 17] = 0x05;
            cpy[24 * j + 18] = data[16 * j + 10];
            cpy[24 * j + 19] = data[16 * j + 11];

            //通道6 --》 ref2
            cpy[24 * j + 20] = 0x00;
            cpy[24 * j + 21] = 0x06;
            cpy[24 * j + 22] = data[16 * j + 12];
            cpy[24 * j + 23] = data[16 * j + 13];
    ...
    // usb录音小板, 8通道数据转6通道（4mic）
    public static byte[] adapeter4Mic2(byte[] data) {
        ...
        while (j < data.length / 16) {
            ...
            cpy[12 * j + 0] = data[16 * j + 4];
            cpy[12 * j + 1] = data[16 * j + 5];

            cpy[12 * j + 2] = data[16 * j + 6];
            cpy[12 * j + 3] = data[16 * j + 7];

            cpy[12 * j + 4] = data[16 * j + 8];
            cpy[12 * j + 5] = data[16 * j + 9];

            cpy[12 * j + 6] = data[16 * j + 10];
            cpy[12 * j + 7] = data[16 * j + 11];
    ...
    public static byte[] adapeter4Mic(byte[] data) {
        ...
        while (j < data.length / 16) {
            ...
            cpy[24 * j + 16] = 0x00;
            cpy[24 * j + 17] = 0x05;
            cpy[24 * j + 18] = data[16 * j + 8];
            cpy[24 * j + 19] = data[16 * j + 9];

            //通道6 --》 ref2
            cpy[24 * j + 20] = 0x00;
            cpy[24 * j + 21] = 0x06;
            cpy[24 * j + 22] = data[16 * j + 10];
            cpy[24 * j + 23] = data[16 * j + 11];
...
//            cpy[24* j + 23] = data[16 * j +15];
    ...
    public static byte[] adapeter1C(byte[] data) {
        int size = data.length / 8;
        byte[] cpy = new byte[size];
        int j = 0;

        while (j < data.length / 16) {
            cpy[2 * j + 0] = data[16 * j + 0];
    ...
    // 6Mic录音板转2mic（取3、4通道）、16bit 转 32bit
    public static byte[] adapeter6MicTo2Mic(byte[] data) {
        int size = data.length * 2;
        byte[] cpy = new byte[size / 8 * 4];
        int j = 0;
        ...
        while (j < data.length / 16) {
            ...
            cpy[16 * j + 0] = 0x00;
            cpy[16 * j + 1] = 0x03;
            cpy[16 * j + 2] = data[16 * j + 4];
            cpy[16 * j + 3] = data[16 * j + 5];

            // 通道4
            cpy[16 * j + 4] = 0x00;
            cpy[16 * j + 5] = 0x04;
            cpy[16 * j + 6] = data[16 * j + 6];
            cpy[16 * j + 7] = data[16 * j + 7];
